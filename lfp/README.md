...
**Consolidation (v4.2).** `src/consolidate.ts::groupViolations` folds violations that share a subject node, or the same invariant across `contains`-siblings, into one **group**; `syncRaised` raises one parent follow-up per group (`covers` lists the violation ids) plus hidden children per violation, and `rankOpen` shows only the parent (tagged `talk-next-covers`). Answering the parent answers every child. The `consolidate-questions` AI function (Talk: `consolidate`, or the `talk-consolidate` button) rewrites the parent into one sentence through the `refine` cue; a refined prompt survives until the group's membership changes. `find-contradictions` (Talk: `contradictions`) raises `raisedBy.kind = 'contradiction'` follow-ups for duplicate-titled nodes of one kind and for rules governing the same thing whose lines disagree.

**Task lifecycle gate (v4.2).** `reality.json.verdicts[taskId]` holds the reviewer's verdict; `applyReality` moves a task `done → verified` (verdict serves-intent and every targeted test pass and fresh) or `blocked` (overfits/unclear). `task-done-without-verdict` and `task-verified-without-green` are kernel checks like any other. Reference shows the verdicts (`ref-verdicts`); `docs/AGENT-RUNTIME.md` §8 records how the reviewer tier is picked.
