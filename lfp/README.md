...
- **Domain** (was Kernel) — three C4-style levels. 1 Context: the system as a bubble, the people outside, other systems. 2 Modules: business domains with their infra, arrows between them. 3 Inside a module: things, rules, events, interface, tests; select a thing to see the rules that govern it; every item can link to the code on GitHub (`props.codeRef`). No level 4.
